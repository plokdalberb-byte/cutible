...
_SESSION: dict[str, Editor | None] = {"editor": None}


def _ed() -> Editor:
    if _SESSION["editor"] is None:
        raise VerbError("no active project", hint="call create_project (or load_project) first")
...
def t_create_project(
    id: str, fps: int = 30, width: int = 1920, height: int = 1080, prompt: str = ""
) -> dict:
    p = Project(
        id=id, fps=fps, width=width, height=height, provenance={"prompt": prompt} if prompt else {}
    )
...
def t_render(output: str, run_qc_after: bool = True, dry_run: bool = False) -> dict:
    ...
    if run_qc_after and not dry_run:
        report = run_qc(
            output,
            expected_duration=_ed().project.duration,
...
def t_qc(file: str, expected_duration: float | None = None) -> dict:
...
def t_build_narrative(project_id: str = "default", index_dir: str = ".cutible/index") -> dict:
    from .ingest import IngestPipeline
    from .ingest.pipeline import IngestConfig
...
def t_reframe_to(asset_id: str, target_aspect: str = "9:16", focus: str = "center") -> dict:
    from .verbs_high import HighLevelVerbs
...
def t_generate_captions(
    track_id: str, style: str = "default", index_dir: str = ".cutible/index"
) -> dict:
...
def t_auto_ducking(voice_track_id: str, music_track_id: str, duck_level: float = 0.15) -> dict:
    from .verbs_high import HighLevelVerbs
...
def t_make_short(topic: str, duration: float = 60.0, index_dir: str = ".cutible/index") -> dict:
    from .index import IndexStore
    from .verbs_high import HighLevelVerbs
...
def t_vlm_review(video_path: str, brief: str = "", model: str = "gemini") -> dict:
...
def t_run_agent_swarm(
    ...
    return orchestrator.run(
        brief=brief, target_duration=target_duration, style=style, index_dir=index_dir
    )
...
TOOL_SCHEMAS = {
    "create_project": (
        _obj({"id": _S(), "fps": _I(), "width": _I(), "height": _I(), "prompt": _S()}, ["id"]),
    ...
    "read": (
        _obj({"zoom": _S(enum=["summary", "outline", "detail"])}, []),
    ...
    "add_asset": (
        _obj(
            {
                "asset_id": _S(),
                "type": _S(enum=["video", "audio", "image", "color"]),
                "uri": _S(),
                "duration": _N(),
                "color": _S(),
            },
    ...
    "add_track": (
        _obj(
            {"track_id": _S(), "kind": _S(enum=["video", "audio", "caption"])}, ["track_id", "kind"]
        ),
        "Add a track.",
    ),
    "add_clip": (
    ...
    "trim": (
        _obj({"clip_id": _S(), "src_in": _N(), "src_out": _N()}, ["clip_id"]),
        "Adjust a clip's source in/out.",
    ),
    "move": (
        _obj({"clip_id": _S(), "timeline_in": _N()}, ["clip_id", "timeline_in"]),
        "Move a clip on its track.",
    ),
    "split": (
        _obj({"clip_id": _S(), "t": _N()}, ["clip_id", "t"]),
        "Split a clip at timeline time t.",
    ),
    "ripple_delete": (_obj({"clip_id": _S()}, ["clip_id"]), "Delete a clip and close the gap."),
    "set_speed": (
        _obj({"clip_id": _S(), "speed": _N()}, ["clip_id", "speed"]),
        "Set clip playback speed.",
    ),
    "set_volume": (
        _obj({"clip_id": _S(), "volume": _N()}, ["clip_id", "volume"]),
        "Set clip volume.",
    ),
    "add_transition": (
        _obj({"clip_id": _S(), "kind": _S(enum=["in", "out"]), "duration": _N()}, ["clip_id"]),
    ...
    "reframe_to": (
        _obj(
            {
                "asset_id": _S(),
                "target_aspect": _S(enum=["9:16", "16:9", "1:1", "4:3"]),
                "focus": _S(enum=["center", "top", "bottom"]),
            },
            ["asset_id"],
        ),
    ...
    "sync_cuts_to_beat": (
        _obj({"track_id": _S(), "audio_track_id": _S()}, ["track_id", "audio_track_id"]),
    ...
    "generate_captions": (
        _obj(
            {
                "track_id": _S(),
                "style": _S(enum=["default", "bold", "minimal", "social"]),
                "index_dir": _S(),
            },
            ["track_id"],
        ),
    ...
    # --- OTIO bridge --- #
    "export_otio": (
        _obj({"output_path": _S()}, ["output_path"]),
        "Export the project as an OpenTimelineIO (.otio) file.",
    ),
    "import_otio": (
        _obj({"otio_path": _S()}, ["otio_path"]),
        "Import an OpenTimelineIO file into the current project.",
    ),
    # --- render farm --- #
    "render_farm": (
        _obj({"output": _S(), "n_workers": _I()}, ["output"]),
        "Render using the distributed render farm.",
    ),
    "render_farm_dry_run": (_obj({}, []), "Show what the render farm would do without rendering."),
}


def _tools_list() -> list[dict]:
    out = []
    for name, _fn in TOOLS.items():
