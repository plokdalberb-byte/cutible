...
from .schema import Asset, AssetType, Clip, Project, TextLayer, TrackKind
...
class FFmpegCompiler:
    ...
    def _input_args(self, clip: Clip, asset: Asset, kind: TrackKind) -> list[str]:
        ...
        if asset.type == AssetType.color:
            return [
                ...
                f"{clip.duration:.6f}",
                "-i",
                f"color=c={asset.color}:s={self.p.width}x{self.p.height}:r={self.p.fps}",
            ]
        if asset.type == AssetType.image:
            return ["-loop", "1", "-t", f"{clip.duration:.6f}", "-i", asset.uri]
        # video / audio file
        return ["-i", asset.uri]
    ...
    def _video_chain(self, idx: int, clip: Clip, asset: Asset, out_label: str) -> str:
        ...
        steps.append(f"scale={target_w}:{target_h}:force_original_aspect_ratio=decrease")
    ...
    def build(self) -> CompiledRender:
        ...
        inputs.append(
            [
                ...
                f"{duration:.6f}",
                "-i",
                f"color=c={p.globals.background}:s={p.width}x{p.height}:r={p.fps}",
            ]
        )
        ...
        for track in p.tracks:
            for clip in track.clips:
                ...
                wants_video = track.kind in (TrackKind.video, TrackKind.caption) and asset.type in (
                    AssetType.video,
                    AssetType.image,
                    AssetType.color,
                )
                wants_audio = asset.type in (AssetType.video, AssetType.audio) and track.kind in (
                    TrackKind.video,
                    TrackKind.audio,
                )
        ...
        if has_audio:
            if len(audio_labels) == 1:
                graph.append(
                    f"[{audio_labels[0]}]aresample={p.render_settings.audio_rate},"
                    f"loudnorm=I={p.globals.loudness_target}:LRA=11:TP=-1.5[aout]"
                )
            else:
                ins = "".join(f"[{lbl}]" for lbl in audio_labels)
                graph.append(
                    f"{ins}amix=inputs={len(audio_labels)}:normalize=0:dropout_transition=0[amixed]"
                )
                graph.append(
    ...
    # ------------------------------------------------------------------ #
    def render(self, out_path: str, *, dry_run: bool = False, quiet: bool = True) -> dict:
        compiled = self.build()
