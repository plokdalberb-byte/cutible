...
from ..schema import (
    Asset, AssetType, Clip, Project, TextLayer, Track, TrackKind,
)
...
class OTIOImporter:
    ...
    def import_file(self, otio_path: str, project_id: Optional[str] = None) -> Project:
    ...
    def _import_timeline(self, timeline: "otio.schema.Timeline",
    ...
    def _import_track(self, otio_track: "otio.schema.Track",
                      project: Project, asset_map: dict[str, str]) -> Optional[Track]:
    ...
    def _import_clip(self, otio_clip: "otio.schema.Clip",
                     project: Project, asset_map: dict[str, str],
                     kind: TrackKind) -> Optional[Clip]:
        ...
        if asset_id not in asset_map and asset_id not in [a.id for a in project.assets]:
            ...
            if uri:
                ...
                project.assets.append(Asset(
                    id=asset_id, type=asset_type, uri=uri,
                ))
    ...
    def _import_text(self, otio_clip: "otio.schema.Clip") -> Optional[TextLayer]:
    ...
    def _import_fallback(self, otio_path: str,
        ...
        for track_data in data.get("tracks", []):
            ...
            for clip_data in track_data.get("clips", []):
                ...
                if uri and asset_id not in [a.id for a in project.assets]:
                    project.assets.append(Asset(
                        id=asset_id, type=AssetType.video, uri=uri,
                    ))
                clip = Clip(
                    id=clip_meta.get("clip_id", clip_data.get("name", "clip")),
                    asset=asset_id, src_in=src_start, src_out=src_end,
