...
class AudioAnalyzer:
    ...
    def estimate_tempo(self, uri: str) -> Optional[float]:
    ...
    def detect_silences(self, uri: str, min_silence: float = 0.5,
                        threshold_db: float = -40.0) -> list[tuple[float, float]]:
    ...
    def _tempo_librosa(self, audio_path: str) -> Optional[float]:
        import librosa
        y, sr = librosa.load(audio_path, sr=self.sample_rate)
    ...
    def _silences_librosa(self, audio_path: str, min_silence: float,
    ...
    def _analyze_ffmpeg(self, audio_path: str) -> list[AudioFeatures]:
        ...
        try:
            ...
            if not features:
                ...
                features.append(AudioFeatures(
                    timestamp=0.0, duration=duration,
                    rms_energy=0.5, peak_db=-20.0,
                ))
            return features
        except Exception:
            return []
    ...
    def _tempo_ffmpeg(self, audio_path: str) -> Optional[float]:
        return 120.0
    ...
    def _extract_audio(self, uri: str) -> Optional[str]:
        try:
            ...
            cmd = [
                ...
                "-ar", str(self.sample_rate), "-ac", "1", tmp.name,
            ]
            proc = subprocess.run(cmd, capture_output=True, text=True, timeout=120,
                                   encoding="utf-8", errors="replace")
